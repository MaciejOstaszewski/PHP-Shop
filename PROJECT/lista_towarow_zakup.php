...
<body>
  ...
  <section>
    ...
    <div class="hideForm">
    <form action="services/nowy_zakup.php?id=<?php echo $_GET['id']; ?>" method="post">
      Podaj towar:<br>
      <div>
        <select name="ware">
          <?php
...
<form action="lista_towarow_zakup.php?id=<?php echo $_GET['id']; ?>" method="post">
  Podaj towar:<br>
  <div>
  ...
  <br><input type="submit" value="Szukaj">
    ...
    if(isset($_POST)){
      $sql = "SELECT * FROM zakup
      JOIN towary
      ON zakup.id_towaru = towary.id_towaru
      JOIN kategorie
      ON towary.id_kategorii = kategorie.id_kategorii
      WHERE id_zamowienia_sklep =".$idZamowienia;
      $sql = $sql . " AND ";
      foreach($_POST as $key => $value){
        ...
        if($key == "id_towaru" || $key == "id_kategorii"){
          ...
          $sql = $sql . " ilosc_towaru BETWEEN '" . $_POST['iloscOD'] . "' AND '".$_POST['iloscDO']."' AND ";
    ...
    if (mysqli_num_rows($result) > 0){
      ...
      while($row = mysqli_fetch_assoc($result)){
        echo "<tr><td>" . $row['id_zakupu'] . "</td>
                  <td><a href='towar_info.php?id=".$row['id_towaru']."'>" . $row['nazwa'] ." ". $row['model'] ."</a></td>
                  <td>" . $row['nazwa_kategorii'] . "</td>
                  <td>" . $row['ilosc_towaru'] . "</td>
                  <td class='deleteRow'>
                  <a href='services/delete_lista_zakup.php?id=".$row['id_zakupu']."
                    &ilosc=".$row['ilosc_towaru']."
                    &id_zamowienia=".$idZamowienia."
                    &towar=".$row['id_towaru']."
                    ' class='delete'><i class='icon-trash'></i></a>
                  </td><td class='editRow'>
                  <a href='edit_lista_towarow_zakup_form.php?id=".$row['id_zakupu']."
                  &ware=".$row['id_towaru']."
                  &quantity=".$row['ilosc_towaru']."
                  &id_zamowienia=".$idZamowienia."
